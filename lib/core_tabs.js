...
var core_tabs = function(spec, my) {
  ...
  tabs_devtools = function(src, args, cb_) {
    ...
    async.parallel({
      url: function(cb_) {
        t.frame.get_dev_tools_url(function(url) {
          return cb_(null, url);
        });
      },
      id: function(cb_) {
        t.frame.get_dev_tools_id(function(url) {
          return cb_(null, url);
        });
      }
    }, cb_);
  ...
  // with event parameters the tab `id` and the `item` clicked.
  // ```
  // @src  {string} source module
