...
var core_tabs = function(spec, my) {
  ...
  var tabs_context_menu;       /* tabs_context_menu(args, cb_); */
  ...
  // ### tabs_context_menu
  //
  // Sets context menu entries for the specified tab. When the entry is called,
  // an event is broadcasted by the core module.
  // ```
  // @src  {string} source module
  // @args {object} { id, procedure }
  // @cb_  {function(err, res)}
  // ```
  tabs_register_context_menu = function(src, args, cb_) {
    var t = my.tabs[args.id];
    if(!t) {
      return cb_(common.err('Invalid tab `id`: ' + args.id,
                            'core_tabs:invalid_tab'));
  ...
  common.method(that, 'tabs_context_menu', tabs_context_menu, _super);
