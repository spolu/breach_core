...
var core_tabs = function(spec, my) {
  ...
  var tabs_context_menu_handler; /* tabs_context_menu_handler(args, cb_); */
  var tabs_default_url;          /* tabs_default_url(args, cb_); */
  ...
  frame_navigation_state = function(frame, state) {
    ...
    if(t) {
      state.entries.forEach(function(n) { 
        if(n.url.href === my.default_url) {
          n.url.protocol = 'breach:';
          n.url.slashes = true;
          n.url.auth = null;
          n.url.host = 'default';
          n.url.port = null;
          n.url.hostname = 'default';
          n.url.hash = null;
          n.url.search = null;
          n.url.query = null;
          n.url.pathname = '';
          n.url.path = '';
          n.url.href = 'breach://default';
        }
        else {
  ...
  tabs_new = function(src, args, cb_) {
    ...
    var target_url = args.url || my.default_url;

    /* `breach://` translation. */
    var breach_url_r = /^breach\:\/\/(.*)$/;
    var breach_url_m = breach_url_r.exec(target_url);
    if(breach_url_m) {
      target_url = my.core_module.core_ui().url_for_ui(breach_url_m[1]);
    }
  ...
  tabs_load_url = function(src, args, cb_) {
    ...
    t.frame.load_url(target_url, cb_);
  };
  ...
  tabs_devtools = function(src, args, cb_) {
    ...
    async.parallel({
      ...
      id: function(cb_) {
        t.frame.get_dev_tools_id(function(id) {
          common.log.out('DEVTOOL ID: ' + id);
          return cb_(null, id);
        });
      }
    }, cb_);
  ...
  // ### tabs_context_menu_handler
  //
  // Registers a context menu hanlder for the calling module. The procecudure
  ...
  // @cb_  {function(err, res)}
  // ```
  tabs_context_menu_handler = function(src, args, cb_) {
    var t = my.tabs[args.id];
    if(!t) {
      return cb_(common.err('Invalid tab `id`: ' + args.id,
                            'core_tabs:invalid_tab'));
  ...
  // ### tabs_default_url
  //
  // Set the tabs default url when a new tab is created without URL
  // ```
  // @src  {string} source module
  // @args {object} { url }
  // @cb_  {function(err, res)}
  // ```
  tabs_default_url = function(src, args, cb_) {
    my.default_url = args.url;
    return cb_();
  };

  // ### tabs_state
  //
  // Retrieves the current tabs state (normlaly sent through push_state)
  // ```
  // @src  {string} source module
  // @args {object} { }
  // @cb_  {function(err, res)}
  ...
  common.method(that, 'tabs_context_menu_handler', tabs_context_menu_handler, _super);
  common.method(that, 'tabs_default_url', tabs_default_url, _super);
