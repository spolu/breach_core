...
var stack = function(spec, my) {
  ...
  init = function(cb_) {
    ...
    my.session.exo_browser().on('frame_loading_start', 
                                frame_loading_start);
    my.session.exo_browser().on('frame_loading_stop', 
                                frame_loading_stop);
  ...
  shortcut_stack_close = function() {
    ...
    if(p.state.entries[p.state.entries.length - 1] &&
       p.state.entries[p.state.entries.length - 1].url) {
  ...
  common.method(that, 'page_for_frame', page_for_frame, _super);
  common.method(that, 'page_for_frame_name', page_for_frame_name, _super);
