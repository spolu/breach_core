    ...
    async.series([
      ...
      function(cb_) {
        ...
        my.module_manager.on('init:list', function(modules) {
          common.log.out('init:list');
          common.log.out(JSON.stringify(modules, null , 2));
        });
        my.module_manager.on('init:check', function(module, status) {
          common.log.out('init:check');
          common.log.out(JSON.stringify(module, null , 2));
          common.log.out(status);
        });
        my.module_manager.on('init:install', function(module) {
          common.log.out('init:install');
          common.log.out(JSON.stringify(module, null , 2));
        });
        my.module_manager.on('init:start', function(module) {
          common.log.out('init:start');
          common.log.out(JSON.stringify(module, null , 2));
        });
        my.module_manager.on('init:fail', function(module, err) {
          common.log.out('init:fail');
          common.log.out(JSON.stringify(module, null , 2));
          common.log.out(err.stack);
        });
      ...
      function(cb_) {
        my.module_manager.init(function(err) {
          ...
          setTimeout(function() {
            my.module_manager.core_emit('init', { foo: 'bar' });
          }, 1000);
          /*
          ...
          */
          /*
          my.module_manager.add_module(
            'breach/mod_test', '0.0.3', false, function(err) {
            if(err) {
