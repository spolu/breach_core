...
var cookie_store = function(spec, my) {
  ...
  load_for_key = function(key, cb_) {
    ...
    my.db.find(query, function(err, ccs) {
      if(err) {
        ...
        common.log.debug('COOKIE LOAD_FOR_KEY: ' + key + 
                          ' [' + typeof key + ']');
        common.log.debug(JSON.stringify(ccs, null, 2));
        return cb_(ccs);
      }
    });
  };
  ...
  add = function(cc) {
    my.db.insert(cc, function(err) {
      if(err)
        ...
        common.log.debug('COOKIE ADDED: ');
        common.log.debug(JSON.stringify(cc, null, 2));
      }
    });
  };
  ...
  remove = function(cc) {
    my.db.remove({ 
      ...
      if(err)
        ...
        common.log.debug('COOKIE REMOVED: ');
        common.log.debug(JSON.stringify(cc, null, 2));
      }
    });
  };
  ...
  update_access_time = function(cc) {
    my.db.update({ 
      ...
      if(err)
        ...
        common.log.out('COOKIE ACCESS TIME UPDATED: ');
        common.log.out(JSON.stringify(cc, null, 2));
      }
    });
  };
  ...
  force_keep_session_state = function() {
    ...
    common.log.out('COOKIE: `force_keep_session_state`');
  };
