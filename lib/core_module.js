...
var core_module = function(spec, my) {
  ...
  init = function(cb_) {
    ...
    /* Core Controls. */
    my.core_tabs = require('./core_controls.js').core_controls({
      core_module: that
    });

    /* ExoSession. */
    my.exo_session = api.exo_session({
      path: my.session.get_data_path(),
      off_the_record: my.session.get_off_the_record()
    });
    /* ExoBrowser. */
    ...
    */
    ...
    /* API. */
    my.session.get_module_manager().core_expose('tabs_new', my.core_tabs.tabs_new);
    my.session.get_module_manager().core_expose('tabs_close', my.core_tabs.tabs_close);
    my.session.get_module_manager().core_expose('tabs_tag', my.core_tabs.tabs_tag);
    my.session.get_module_manager().core_expose('tabs_untag', my.core_tabs.tabs_untag);
    my.session.get_module_manager().core_expose('tabs_show', my.core_tabs.tabs_untag);
    my.session.get_module_manager().core_expose('tabs_get', my.core_tabs.tabs_get);

    my.session.get_module_manager().core_expose('control_set', my.core_controls.control_set);
    my.session.get_module_manager().core_expose('control_unset', my.core_controls.control_unset);
    my.session.get_module_manager().core_expose('control_dimension', my.core_controls.control_dimension);
    my.session.get_module_manager().core_expose('control_focus', my.core_controls.control_focus);
