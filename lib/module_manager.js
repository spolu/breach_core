...
var module_manager = function(spec, my) {
  ...
  stop_module = function(module, cb_) {
    /* TODO(spolu): RPC call for kill. */
    if(my.modules[module.module_id]) {
      my.modules[module.module_id].process.removeAllListeners();
      my.modules[module.module_id].kill();
  ...
  // ### add_module
  //
  // Indenpotemt. Adds the module to the module database + attemps to install it
