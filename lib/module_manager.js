...
var module_manager = function(spec, my) {
  ...
  install_module = function(module, cb_) {
    var version = 'v' + module.version;
    factory.log().out('INSTALL');
    ...
    repo.tags(function(err, data) {
      ...
      if(!found) {
        return cb_(common.error('Module not found: ' + module.module_id + ' ' +
                                'v' + module.version, 'module_notfound'));
      }
      var module_path = path.join(my.modules_path, 
                                  module.module_id.split('/')[0]);
      mkdirp(module_path, function(err) {
        if(err) {
          return cb_(err);
        }
        var tarball_path = path.join(my.modules_path, 
                                     module.module_id + '@' + module.version +
                                     '.tar.gz');
        https.get(found.tarball_url, function(res) {
          if(res.headers.location) {
            var file = fs.createWriteStream(tarball_path);
            ...
            return cb_(common.error('Unexpected GitHub header', 
                                    'github_error'));
          }
        });
      });
  ...
  common.method(that, 'install_module', install_module, _super);
