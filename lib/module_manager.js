...
var module_manager = function(spec, my) {
  ...
  var settings_get;           /* settings_get(path, cb_); */
  var settings_post;          /* settings_post(path, settings, cb_); */
  ...
  // @cb_      {function(err, module)}
  // ```
  settings_post = function(path, settings, cb_) {
    my.core_module.procedures.settings_store(path, settings, function(err, settings) {
      cb_(err, {settings: settings});
    });
  };
  ...
  common.method(that, 'settings_post', settings_post, _super);
