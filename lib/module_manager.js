...
var module_manager = function(spec, my) {
  ...
  expand_path = function(path) {
    ...
    // npm:name[@version] 
    var npm_r = /^npm\:([a-zA-Z0-9\-_\.]+)(@[a-zA-Z0-9\-_\.]+){0,1}$/;
    var npm_m = npm_r.exec(path);
    if(npm_m) {
      return {
        type: 'npm',
        name: npm_m[1],
        version: npm_m[2] ? npm_m[2].substr(1) : null
      };
    }
  ...
  // one. If it's an npm path, then it makes sure the version is valid
  // (if specified) otherwise it finds the latest version.
  //
  // ```
  // @path {string} a module path
  ...
  augment_path = function(path, cb_) {
    ...
    if(p.type === 'github') {
      ...
      if(p.version && !semver.valid(p.version)) {
        return cb_(common.err('Invalid `path` version:' + p.version + ' (does not follow semver)',
                       'module_manager:invalid_path'));
      } else if(!p.version) {
        npm.commands.view([p.name], function(err, data) {
          if(err) {
            return cb_(common.err('Failed to fetch npm module data for \'' + p.name + '\'',
                                  'module_manager:failed_npm_fetch'));
          }
          
          var versions = Object.keys(data);

          if(versions && versions.length) {
            cb_(null, p.type + ':' + p.name + '@' + versions[0]);
          } else {
            cb_(common.err('Invalid module data received from npm for \'' + p.name + '\'',
                           'module_manager:invalid_npm_data'));
          }
        });
      } else {
        return cb_(null, p.type + ':' + p.name + '@' + semver.clean(p.version)); 
      }
        ...
        async.eachSeries(Object.keys(modules), function(m, cb_) {
          if(modules[m]) {
            common.log.out('[module_manager] Attempting auto_update of: ' + 
                           modules[m].path);
            /* TODO(spolu): Updating agressively may leave the module in a */
      ...
      async.series([
        ...
        function(cb_) {
          my.gig.get('core', 'module', my.gig_path, function(err, modules) {
            ...
            var adding = expand_path(path);
            var arr = Object.keys(modules);

            for(var i = 0; i < arr.length; i ++) {
              var m = modules[arr[i]];
              var current = expand_path(m.path);

              if(!force && 
                current.type === adding.type) {

                if(m.path === path ||
                   (adding.type === 'github' &&
                    current.owner === adding.owner && 
                    current.name === adding.name) ||
                   (adding.type === 'npm' &&
                    current.name === adding.name &&
                    current.version === adding.version) ||
                   (adding.type === 'local' &&
                    m.path === path)) {
        ...
        function(cb_) {
          ...
          if(expanded.type === 'local') {
            var package_path = require('path')
              .join(expanded.path, 'package.json');
          ...
          if(expanded.type === 'npm') {
            package_json = expanded;
            return cb_();
          }
        },
      ...
      function(cb_) {
        ...
        fs.stat(storage_path(path), function(err, stat) {
          ...
          if(err && err.code !== 'ENOENT') {
            ...
            if(expanded.type === 'github') {
              var options = {
                url: 'https://api.github.com' + 
                     '/repos/' + 
                     expanded.owner + '/' + expanded.name + 
                     '/tarball/' + expanded.tag,
                headers: {
                  'User-Agent': 'Mozilla/5.0'
                }
              }
            ...
            else if (expanded.type === 'npm') {
              npm.commands.view([expanded.name + '@' + expanded.version], function (err, data) {
                if(err) {
                  return cb_(err);
                }

                common.log.debug(data.dist.tarball);
                return cb_(null, data);
              });
            }
            else if(expanded.type === 'local') {
              return cb_(err);
            }
          }
          else {
  ...
  // provided must be a fully epxlicited (tag) path of an existing module.
  //
  ...
  update = function(path, cb_) {
    ...
    async.series([
      ...
      /* We finally mark updated running modules as need_restar. */
      function(cb_) {
        if(my.running_modules[module.name]) {
