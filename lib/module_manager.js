...
var module_manager = function(spec, my) {
  ...
  install_module = function(module, cb_) {
    ...
    async.waterfall([
      ...
      function(cb_) {
        ...
        factory.log().out('INSTALL: ' + module.module_id + 
                          '@' + module.version);
      ...
      function(gh_url, cb_) {
        factory.log().out('NPM INSTALL: ' + gh_url);
  ...
  start_module = function(module, cb_) {
    factory.log().out('Module `start_module`: ' + module.module_id);
    ...
    var p = child_process.fork(module_path(module), ['--raw']);
    ...
    p.on('exit', function(code) {
      ...
      factory.log().out('Module exited unexpectedly: ' + module.module_id);
      p.removeAllListeners();
      if(my.modules[module.module_id].restart < 3) {
        factory.log().out('Restarting: ' + module.module_id);
  ...
  stop_module = function(module, cb_) {
    factory.log().out('Module `stop_module`: ' + module.module_id);
    if(my.modules[module.module_id]) {
      ...
      my.pending_modules[module.module_id].process.on('exit', function() {
        factory.log().out('Module exited after `stop_module`: ' + 
                          module.module_id);
      ...
      setTimeout(function() {
        if(my.pending_modules[module.module_id]) {
          factory.log().out('Module force kill: ' + module.module_id);
  ...
  kill = function(cb_) {
    ...
    async.each(running_modules, function(module, cb_) {
      ...
      factory.log().out('All modules stopped.');
      return cb_();
    });
  };
