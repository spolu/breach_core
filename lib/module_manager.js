...
var module_manager = function(spec, my) {
  ...
  dispatch = function(msg) {
    ...
    switch(msg.hdr.typ) {
      ...
      case 'rpc_call': {
        ...
        if(my.running_modules[msg.dst] && my.running_modules[msg.hdr.src]) {
      ...
      /* ```                                                                  */
      case 'rpc_reply': {
        if(my.running_modules[msg.src]) {
          my.running_modules[msg.src].process.send(msg);
        }
        break;
      }
    }
  ...
  remove = function(path, cb_) {
    ...
    async.series([
      ...
      function(cb_) {
        if(my.running_modules[path]) {
          kill_module(path, cb_);
        }
        else {
          return cb_();
        }
  ...
  run_module = function(path, cb_) {
    ...
    async.series([
      ...
      function(cb_) {
        common.log.out('`start_module`: ' + path);
        my.running_modules[path] = my.running_modules[path] || {
          process: null,
          path: path,
          restart: 0,
          registrations: []
        ...
        my.running_modules[path].process = p;
        ...
        p.on('exit', function(code) {
          ...
          delete my.running_modules[path].process;

          if(my.running_modules[path].restart < 3) {
            common.log.out('Restarting: ' + module.module_id);
            my.running_modules[path].restart++;
            start_module(path, function() {});
          }
          else {
            /* After 3 restarts we stop restarting the module. */
            delete my.running_modules[path];
          }
        });
  ...
  stop_module = function(module, cb_) {
    ...
    if(my.running_modules[module.module_id]) {
      dispatch({
        hdr: { 
          typ: 'rpc_call', 
      ...
      my.pending_modules[module.module_id] = my.running_modules[module.module_id];
      delete my.running_modules[module.module_id];
  ...
  kill = function(cb_) {
    ...
    for(var path in my.running_modules) {
      if(my.running_modules.hasOwnProperty(path)) {
        arr.push(my.running_modules[module_id]);
      }
    }
