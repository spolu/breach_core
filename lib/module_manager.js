  ...
  augment_path = function(path, cb_) {
    ...
    if(p.type === 'github') {
      ...
      if (p.version && !semver.valid(p.version)) {
        cb_(common.err('Invalid `path` version:' + p.version + ' (does not follow semver)',
                       'module_manager:invalid_path'));
      }

      cb_(null, p.name + (p.version ? '@' + semver.clean(p.version) : '')); 
    }
        ...
        fs.stat(storage_path(path), function(err, stat) {
          ...
          if(err && err.code !== 'ENOENT') {
            ...
            if(expanded.type === 'github') {
              ...
              // TODO: install - not sure how yet.. does npm.commands.install allow a path? or will it install in the right place?
            }
