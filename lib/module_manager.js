    ...
    if(p.type === 'github') {
      ...
      if(p.version && !semver.valid(p.version)) {
        ...
        npm.commands.view(p.name, function(err, data) {
          ...
          var meta = normalize(data);

          cb_(null, p.name + '@' + meta.version);
        });
      } else {
        return cb_(null, p.name + '@' + semver.clean(p.version)); 
      }
        ...
        fs.stat(storage_path(path), function(err, stat) {
          ...
          if(err && err.code !== 'ENOENT') {
            ...
            if(expanded.type === 'github') {
              ...
              /* TODO(knownasilya): install - not sure how yet..
              does npm.commands.install allow a path? or will it install in the right place? */
            }
