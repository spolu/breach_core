...
var core_ui = function(spec, my) {
  ...
  var post_modules_output;       /* post_modules_output(req, res, next); */
  ...
  // ### post_modules_output
  //
  // Express route to retrieve a module output
  // ```
  // POST /module/output
  // ```
  post_modules_output = function(req, res, next) {
    var path = req.param('path');
    my.session.module_manager().output(path, function(err, output) {
      if(err) {
        return next(err);
      }
      return res.json({ 
  ...
  init = function(cb_) {
    ...
    my.app.post('/modules/output', post_modules_output);
    ...
    my.session.module_manager().on('state_change', function() {
      socket_push('modules');
    });
