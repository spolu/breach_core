...
var core_ui = function(spec, my) {
  ...
  var url_for_ui;               /* url_for_ui(type, hash); */
    ...
    async.waterfall([
      function(cb_) {
        switch(cmd) {
          ...
          case 'settings_get': {
            my.session.module_manager().settings_get(path, cb_);
            break;
          }
          case 'settings_post': {
            my.session.module_manager().settings_post(path, req.param('settings'), cb_);
            break;
          }
          default: {
  ...
  // ```
  url_for_ui = function(type, hash) {
    hash = hash || '';
    switch(type) {
      case 'modules': {
        return my.base_url + 'modules/' + hash;
      }
      default: {
